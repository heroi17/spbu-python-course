...
class Matrix:
    ...
    def __add__(self, other: "Matrix") -> "Matrix":
        ...
        return Matrix([[self._Data[i][j] + other._Data[i][j] for j in range(self._Width)] for i in range(self._Height)])
    ...
    def __mul__(self, other: "Matrix") ->"Matrix":
        ...
        return Matrix(
                    [
                     [sum(self._Data[i][j] * other._Data[j][k] for j in range(self._Width))
...
class Vector(Matrix):
    ...
    def angle(first: "Vector", second: "Vector") -> float:
        ...
        if (length1 == 0 or length2 == 0):
            raise ZeroDivisionError("Cannot find angle between vectors, where one of them is 0-vector.")
